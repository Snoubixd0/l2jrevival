...
import com.l2jrevival.gameserver.model.actor.instance.Cubic;
import com.l2jrevival.gameserver.model.actor.instance.Player;
import com.l2jrevival.gameserver.model.actor.instance.Servitor;
import com.l2jrevival.gameserver.model.actor.instance.SiegeSummon;
...
public class L2SkillSummon extends L2Skill
{
	...
	public boolean checkCondition(Creature activeChar)
	{
		if (activeChar instanceof Player)
		{
			...
			else
			{
				...
				{
					player.sendMessage("You can summon only one agathion.");
					return false;
				}
			}
		}
	...
	public void useSkill(Creature caster, WorldObject[] targets)
	{
		...
		if (_isAgathion)
		{
			// player attempting to summon more than one agathion
			if (activeChar.getAgathion() != null || activeChar.isMounted())
			{
				return;
			}
			L2AgathionInstance agathion;
			NpcTemplate agathionTemplate = NpcTable.getInstance().getTemplate(_npcId);
			
			if (agathionTemplate == null)
			{
				_log.warning("Summon agathion attempt for nonexisting NPC ID: " + _npcId + ", skill ID: " + getId());
				return;
			}
			agathion =  new L2AgathionInstance(IdFactory.getInstance().getNextId(),agathionTemplate,activeChar);
			agathion.setHeading(activeChar.getHeading());
			agathion.setRunning();
			activeChar.setAgathion(agathion);
			agathion.spawnMe(activeChar.getX() + 20, activeChar.getY() + 20, activeChar.getZ());
			return;
		}
