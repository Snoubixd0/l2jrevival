...
import com.l2jrevival.gameserver.instancemanager.VIPINFO;
...
import com.l2jrevival.gameserver.network.serverpackets.AutoAttackStart;
import com.l2jrevival.gameserver.network.serverpackets.AutoAttackStop;
import com.l2jrevival.gameserver.network.serverpackets.OpenUrl;
import com.l2jrevival.gameserver.network.serverpackets.TutorialCloseHtml;
import com.l2jrevival.gameserver.network.serverpackets.TutorialShowHtml;
...
public class AdminCustom implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.equals("admin_tournament"))
		{
			if (ArenaTask._started)
			{
				...
				activeChar.sendMessage("SYS: You Finished Tournament");
			}
			else
			{
				_log.info("----------------------------------------------------------------------------");
				...
				initEventArena();
				_arena_manual = true;
				activeChar.sendMessage("SYS: You activated Tournament");
			}
		}
		else if (command.equals("admin_partyzone"))
		{
			if (PartyZoneTask._started)
			{
				...
				activeChar.sendMessage("SYS: You Finished "+Config.NAME_EVENT+"");
			}
			else
			{
				_log.info("----------------------------------------------------------------------------");
				...
				initPartyZone();
				_partytask_manual = true;
				activeChar.sendMessage("SYS: You activated "+Config.NAME_EVENT+"");
			}
		}
			...
			VoteZone();
			activeChar.sendMessage("SYS: You used the admin_votezone command..");
		} 
		else if (command.equals("admin_spawn_phantomtown"))
		{
			...
			ThreadPool.schedule(new Runnable()
			{
				...
				public void run()
				{
					...
					activeChar.sendMessage("SYS: You activated phanton town...");
				}
		...
		else if (command.equals("admin_spawn_phantomtvt"))
		{
			...
			ThreadPool.schedule(new Runnable()
			{
				...
				public void run()
				{
					...
					activeChar.sendMessage("SYS: You have activated Phantom TvT...");
				}
		...
		else if (command.equals("admin_spawn_phantomfarm"))
		{
			...
			ThreadPool.schedule(new Runnable()
			{
				...
				public void run()
				{
					...
					activeChar.sendMessage("SYS: You activated phanton farm...");
				}
		...
		else if (command.equals("admin_spawn_phantom"))
		{
			...
			ThreadPool.schedule(new Runnable()
			{
				...
				public void run()
				{
					...
					activeChar.sendMessage("SYS: You activated phanton PvP...");
				}
		...
		else if (command.equals("admin_spawn_archer"))
		{
			...
			ThreadPool.schedule(new Runnable()
			{
				...
				public void run()
				{
					...
					activeChar.sendMessage("SYS: You activated Phantom Archers...");
				}
	...
	private static void initEventArena()
	{
		ThreadPool.schedule(new Runnable()
		{
			...
			public void run()
			{
				com.l2jrevival.events.ArenaTask.SpawnEvent();
			}
	...
	private static void finishEventArena()
	{
		ThreadPool.schedule(new Runnable()
		{
			...
			public void run()
			{
				com.l2jrevival.events.ArenaTask.finishEvent();
			}
	...
	private static void initPartyZone()
	{
		ThreadPool.schedule(new Runnable()
		{
			...
			public void run()
			{
				com.l2jrevival.events.PartyZoneTask.SpawnEvent();
			}
		}, 1);
	}
	...
	private static void finishPartyZone()
	{
		ThreadPool.schedule(new Runnable()
		{
			...
			public void run()
			{
				com.l2jrevival.events.PartyZoneTask.finishEvent();
			}
		}, 1);
	}
	...
	public static final void onVIPLink(Player player, String request) {
		...
		try {
			...
			if (val == 1 && !VipManager.getInstance().hasVipPrivileges(player.getObjectId())) {
				...
				player.sendMessage("SYS: YOU CAN NOT DO THAT !");
			} 
