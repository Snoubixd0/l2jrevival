...
public class VoicedAutoPotion implements IVoicedCommandHandler {
    ...
    public boolean useVoicedCommand(String command, Player activeChar, String params) {
        ...
        if (command.equals("acpon"))
        {
			...
			{
				activeChar.sendMessage("It is not allowed to use Auto Potion while dead.");
				return false ;
			}
			if(activeChar.isInOlympiadMode())
			{
				activeChar.sendMessage("It is not permitted to use Auto Potion in the Olympiads.");
				return false ;
			}
...
//                               Checking our health
                                if ((activeChar.getStatus().getCurrentHp() / activeChar.getMaxHp()) * 100 < ACP_HP_LVL && ACP_HP)
                                {
                                   ...
                                   if(activeChar.isDead())
                                   {
                               	   activeChar.sendMessage("Dead Auto Potion is not allowed.");
                                	   return ;
                                   }
                                   if(activeChar._inEventCTF && activeChar._haveFlagCTF)
                   					{
                   					activeChar.sendMessage("It is not allowed to use Auto Potion with a CTF weapon.");
                   					return ;
                   					}
                                   if(activeChar.isInOlympiadMode())
                                   {
                                	   activeChar.sendMessage("Auto Potion Olympiadas are not allowed.");
                                	   return ;
                                   }
                                    if (handlerHP != null)
                                    {
