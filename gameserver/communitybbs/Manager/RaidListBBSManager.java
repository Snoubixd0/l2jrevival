...
import com.l2jrevival.gameserver.communitybbs.BB.GrandBossList;
import com.l2jrevival.gameserver.communitybbs.BB.HeroList;
import com.l2jrevival.gameserver.communitybbs.BB.RaidList;
...
import com.l2jrevival.gameserver.network.serverpackets.ShowBoard;
...
public class RaidListBBSManager extends BaseBBSManager
{
	...
	public void parseCmd(String command, Player player)
	{
		...
		if (command.equals("_bbstop") | command.equals("_bbshome"))
		{
			...
			if (content != null)
			{
				...
				content = content.replace("%runspeed%", String.valueOf(player.getMoveSpeed()));
		...
		else if (command.startsWith("_bbstop;"))
		{
			...
			switch (file)
			{
				...
				case "boss":
					...
					break;
				case "stats":
					content = content.replace("%online%", " " + World.getInstance().getAllPlayers());
	...
	{
		int counter = 0;
		for (Player onlinePlayer : World.getInstance().getAllPlayers().values())
		{
		...
		String realOnline = "<tr><td fixwidth=11></td><td FIXWIDTH=280>Players Active</td><td FIXWIDTH=470><font color=26e600>" + counter + "</font></td></tr>" + "<tr><td fixwidth=11></td><td FIXWIDTH=280>Players Shops</td><td FIXWIDTH=470><font color=26e600>" + (World.getInstance().getAllPlayers().values()) + "</font></td></tr>";
		return realOnline;
	}
