...
public class Q642_APowerfulPrimevalCreature extends Quest
{
	...
	private static final int ANCIENT_EGG = 35657;
	...
	{
		super(642, "A Powerful Golden Creature");
		
		setItemsIds(DINOSAUR_TISSUE, DINOSAUR_EGG);
		
		addStartNpc(1004); // Valentina
		addTalkId(1004);
		
		// Dinosaurs + egg
		addKillId(35658, ANCIENT_EGG);
	}
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.contains("event_"))
		{
			if (st.getQuestItemsCount(DINOSAUR_TISSUE) >= 150 && st.hasQuestItems(DINOSAUR_EGG))
			{
				...
				st.takeItems(DINOSAUR_TISSUE, 1500);
				st.takeItems(DINOSAUR_EGG, 10);
