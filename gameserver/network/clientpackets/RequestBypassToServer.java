...
import com.l2jrevival.gameserver.BalancerEdit;
...
import com.l2jrevival.gameserver.handler.IAdminCommandHandler;
...
import com.l2jrevival.gameserver.handler.voicedcommandhandlers.VoicedAutoFarm;
...
import com.l2jrevival.gameserver.model.actor.instance.Gatekeeper;
import com.l2jrevival.gameserver.model.actor.instance.OlympiadManagerNpc;
...
import com.l2jrevival.gameserver.util.ChangeAllyNameLog;
import com.l2jrevival.gameserver.util.ChangeClanNameLog;
import com.l2jrevival.gameserver.util.ChangeNameLog;
import com.l2jrevival.Dungeon.InstanceManager;
...
public final class RequestBypassToServer extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		try
		{
			if (_command.startsWith("admin_"))
			{
				...
				ach.useAdminCommand(_command, activeChar);
						...
						else if (_command.startsWith("_autofarm"))
						{
							...
							{
								final boolean isSameIpAutofarming = World.getInstance()
					    				.getPlayers()
					    				.stream()
					    				.filter(sameIpPlayer -> sameIpPlayer.getIpAddress() != null && sameIpPlayer.isAutoFarm() && sameIpPlayer.getObjectId() != activeChar.getObjectId() && sameIpPlayer.getIpAddress().equals(activeChar.getIpAddress()))
					    				.findAny()
					    				.isPresent();
					    		
								if (isSameIpAutofarming) 
								{
									activeChar.sendMessage("Another player is already auto-farming from your ip address.");
									VoicedAutoFarm.showAutoFarm(activeChar);
									return;
								}
			...
			if (_command.startsWith("_radiusAutoFarm"))
			{
				...
				try
				{
						...
						
					{
						...
						activeChar.sendPacket(new ExShowScreenMessage("Auto Farm Range: " + activeChar.getRadius(), 3 * 1000, SMPOS.TOP_CENTER, false));
					}
				...
				try
				{
					...
					if (activeChar.destroyItemByItemId("Name Change", activeChar.getNameChangeItemId(), 1, null, true))
					{
						...
						for (Player gm : World.getAllGMs())
							gm.sendPacket(new CreatureSay(0, Say2.SHOUT, "[Name]", activeChar.getName() + " changed the name to [" + name + "]"));
			...
			else if (_command.startsWith("_clan_name_"))
			{
				...
				try
				{
					...
					if (activeChar.destroyItemByItemId("", activeChar.getClanNameChangeItemId(), 1, null, true))
					{
						...
						for (Player gm : World.getAllGMs())
							gm.sendPacket(new CreatureSay(0, Say2.SHOUT, "[ClanName]", activeChar.getName() + " changed the clan name to [" + name + "]"));
			...
			else if (_command.startsWith("_ally_name_"))
			{
				...
				try
				{
					...
					if (activeChar.destroyItemByItemId("", activeChar.getAllyNameChangeItemId(), 1, null, true))
					{
						...
						for (Player gm : World.getAllGMs())
							gm.sendPacket(new CreatureSay(0, Say2.SHOUT, "[AllyName]", activeChar.getName() + " changed Ally's name to [" + name + "]"));
				...
				try {
					if (activeChar.getBaseClass() != activeChar.getClassId().getId()) {
						activeChar.sendMessage("SYS: You need to be in your Base Class to use this item.");
			...
			{
				final StringTokenizer stringTokenizer = new StringTokenizer(_command, " ");
				final BypassHandler bypassHandler = BypassManager.getInstance().getBypassHandler(stringTokenizer.nextToken());
				if (bypassHandler == null)
					return;
				
				bypassHandler.handleCommand(activeChar, _command);
			}
		}
		catch (Exception e)
		{
	...
	private static void ClassChangeCoin(Player player, String command) {
		...
		if (type.equals("Duelist")) {
			if (player.getClassId().getId() == 88) {
				player.sendMessage("Sorry, you are already with the Class " + nameclasse + ".");
				return;
			} 
	...
	public static void Finish(Player activeChar)
	{
		...
		for (Player gm : World.getAllGMs())
			gm.sendPacket(new CreatureSay(0, Say2.SHOUT, "Chat Manager", activeChar.getName() + " just changed your Base Class."));
