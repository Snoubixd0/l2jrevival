...
import com.l2jrevival.events.PartyZoneTask;
import com.l2jrevival.events.TvT;
import com.l2jrevival.gameserver.communitybbs.Manager.MailBBSManager;
...
import com.l2jrevival.gameserver.handler.voicedcommandhandlers.VoicedDonate;
...
import com.l2jrevival.gameserver.instancemanager.NewsHtml;
...
import com.l2jrevival.gameserver.instancemanager.VIPFreeHTML;
//import com.l2jrevival.gameserver.instancemanager.VIPFreeHTML;
...
import com.l2jrevival.gameserver.network.L2GameClient.GameClientState;
...
import com.l2jrevival.gameserver.network.serverpackets.Die;
import com.l2jrevival.gameserver.network.serverpackets.EtcStatusUpdate;
import com.l2jrevival.gameserver.network.serverpackets.ExMailArrived;
...
import com.l2jrevival.gameserver.network.serverpackets.OpenUrl;
import com.l2jrevival.gameserver.network.serverpackets.PlaySound;
import com.l2jrevival.gameserver.network.serverpackets.PledgeShowMemberListAll;
import com.l2jrevival.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import com.l2jrevival.gameserver.network.serverpackets.PledgeSkillList;
import com.l2jrevival.gameserver.network.serverpackets.PledgeStatusChanged;
...
import com.l2jrevival.gameserver.util.HWID;
import com.l2jrevival.gameserver.util.IPLog;
	...
	public static void Announce(Player activeChar)
	{
		if (Config.ANNOUNCE_HERO_ON_ENTER && activeChar.isHero())
		{
			...
			if (exists)
			{
				...
				Announcement.Announce("The Hero " + activeChar.getName() + " of the class " + className + " is online!");
			}
				...
				Announcement.Announce("The Hero " + activeChar.getName() + " is online!");
		...
		if (clan != null)
		{
			...
			if (clanHall != null)
			{
				if ((activeChar.getObjectId() == activeChar.getClan().getLeaderId() && Config.ANNOUNCE_HALL_OWNERS))
					Announcement.Announce(activeChar.getName() + ", owner of " + clanHall.getName() + " of " + clanHall.getLocation() + " is online!");
			}
		}
			...
			if (activeChar.getClan() != null)
			{
				if (activeChar.getClan().getLeaderName().equals(activeChar.getName()))
				{
					if (CastleManager.getInstance().getCastleByOwner(activeChar.getClan()) != null)
						Announcement.Announce("The Lord " + activeChar.getName() + " owner of the Castle " + CastleManager.getInstance().getCastleByOwner(activeChar.getClan()).getName() + " esta online!");
				}
			}
		}
